...
Adafruit_SSD1351 oled = Adafruit_SSD1351(
    DISPLAY_WIDTH, DISPLAY_HEIGHT, &SPI, CS_PIN_D7, DC_PIN_D6, RST_PIN_D0);
 ...
 */
void dosType(const char* text, int delay_ms = TYPE_DELAY_NORMAL, uint16_t color = DOS_YELLOW) {
...
void displayDOSStartupAnimation() {
    ...
    // Boot sequence messages with authentic DOS styling
    ...
    dosType("Display:SSD1351 [OK]", TYPE_DELAY_FAST);
    dosNewLine();
    dosType("MCU:ESP32-S3R8 [OK]", TYPE_DELAY_FAST);
    dosNewLine();
    dosType("CPU:240MHz [OK]", TYPE_DELAY_FAST);
    dosNewLine();
    dosType("Memory:8MB PSRAM [OK]", TYPE_DELAY_NORMAL);
    dosNewLine();
    delay(LINE_DELAY);
    ...
    dosType("BYTE-90 OS v1.01", TYPE_DELAY_NORMAL, DOS_AMBER);
    dosNewLine();
    delay(LINE_DELAY);
